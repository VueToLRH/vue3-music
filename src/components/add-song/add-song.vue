...
export default {
  ...
  setup() {
    const visible = ref(false) // 是否显示添加歌曲
    const query = ref('')
    ...
    const searchHistory = computed(() => store.state.searchHistory) // 获取搜索历史歌曲
    const playHistory = computed(() => store.state.playHistory) // 获取历史播放歌曲
